...
import { Globe, WifiOff, Settings2, Zap, Monitor, Smartphone, Plug } from 'lucide-react';
          ...
          <tbody className="divide-y divide-slate-200">
            {onus.map(onu => (
              <tr key={onu.id} className="hover:bg-blue-50/20 transition-colors">
                ...
                <td className="px-3 py-2 text-center border-r border-slate-100">
                  <button className="inline-flex items-center gap-1 bg-[#1a73e8] hover:bg-blue-700 text-white text-[10px] font-black py-1 px-3 rounded shadow-sm uppercase tracking-tight">
                     View
                  </button>
                </td>
...
const getSignalColor = (signal: number) => {
  if (signal > -18) return 'bg-[#22c55e]'; // Good
  if (signal > -22) return 'bg-[#eab308]'; // Warning
  if (signal > -25) return 'bg-[#f97316]'; // Danger
