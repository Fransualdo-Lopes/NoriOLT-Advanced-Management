...
import { 
  ...
  Bell, LogOut, Menu, X, ShieldCheck, Zap, User as UserIcon, Settings
} from 'lucide-react';
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4 lg:gap-8 flex-1">
            ...
            <nav className="hidden lg:flex items-center gap-1">
              ...
              <div className="relative">
                ...
                {isSettingsOpen && (
                  <div className="absolute top-full left-0 mt-2 w-48 bg-[#0f172a] border border-slate-800 rounded-xl shadow-2xl py-2 animate-in fade-in slide-in-from-top-2">
                    {['general', 'users', 'presets', 'billing'].map(sub => (
                       <button 
                         key={sub}
                         onClick={() => handleNavClick(sub === 'presets' ? 'presets' : 'settings')}
                         className="w-full text-left px-4 py-2.5 text-xs font-bold text-slate-400 hover:text-white hover:bg-white/5 uppercase tracking-widest transition-colors"
                       >
                         {t[sub as keyof typeof t] || sub}
                       </button>
                    ))}
                  </div>
                )}
          ...
          <div className="flex items-center gap-3 xl:gap-4 shrink-0">
            <div className="flex items-center gap-0.5 bg-slate-800/50 rounded-md p-0.5 border border-slate-700">
               <button onClick={() => setLanguage('en')} className={`px-1.5 py-0.5 rounded text-[9px] font-bold transition-all ${language === 'en' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>EN</button>
               <button onClick={() => setLanguage('pt')} className={`px-1.5 py-0.5 rounded text-[9px] font-bold transition-all ${language === 'pt' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>PT</button>
            </div>
            ...
            <div className="hidden xl:flex items-center gap-2 bg-slate-800/80 border border-slate-700 px-3 py-1.5 rounded-full text-[10px] font-bold text-blue-400 uppercase tracking-widest">
              <ShieldCheck size={12} /> {user?.level || 'Level: Operator'}
            </div>
            ...
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 bg-slate-800 rounded-full flex items-center justify-center border border-slate-700 text-slate-400 hover:text-blue-400 transition-colors cursor-pointer" title={`${user?.username} - ${user?.role}`}>
                <UserIcon size={18} />
              </div>
              <button onClick={onLogout} className="bg-slate-800 p-2 rounded-full border border-slate-700 hover:bg-slate-700 transition-colors group" title="Sair">
      ...
      <footer className="bg-white border-t border-slate-200 py-4 mt-auto shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)]">
        <div className="container mx-auto px-4 flex flex-col md:flex-row justify-between items-center gap-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
           <div className="flex items-center gap-6">
             <div className="flex items-center gap-2"><span className="text-slate-900 font-black">NORI-OS</span><span className="text-slate-300">V3.51.0</span></div>
             <div className="flex items-center gap-1.5 text-blue-500 bg-blue-50 px-2 py-0.5 rounded border border-blue-100"><Activity size={10} /> ENGINE: ACTIVE</div>
           </div>
           <div className="text-center hidden lg:block text-slate-300">© 2025 NORI-OLT ADVANCED PLATFORMS. PRECISE NETWORK ENGINEERING.</div>
        </div>
      </footer>
    </div>
  );
